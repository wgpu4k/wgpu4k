 ...
 * * }
 */
object WGPUSurfaceDescriptorFromAndroidNativeWindow {
    ...
    private val `window$LAYOUT`: AddressLayout = `$LAYOUT`.select(groupElement("window")) as AddressLayout
     ...
     * * }
     */
    fun `window$layout`(): AddressLayout {
        return `window$LAYOUT`
    }

    private const val `window$OFFSET`: Long = 16
     ...
     * * }
     */
    fun window(struct: MemorySegment): MemorySegment {
     ...
     * * }
     */
    fun window(struct: MemorySegment, fieldValue: MemorySegment?) {
        struct.set(
            `window$LAYOUT`, `window$OFFSET`,
            fieldValue!!
        )
    }
