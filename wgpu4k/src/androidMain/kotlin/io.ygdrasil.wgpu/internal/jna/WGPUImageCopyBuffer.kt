...
object WGPUImageCopyBuffer {
    ...
    private val `layout$LAYOUT` = `$LAYOUT`.select(groupElement("layout")) as GroupLayout
     ...
     * * }
     */
    fun `layout$layout`(): GroupLayout {
        return `layout$LAYOUT`
    }

    private const val `layout$OFFSET`: Long = 8
    ...
    fun layout(struct: MemorySegment?, fieldValue: MemorySegment?) {
        MemorySegment.copy(
            fieldValue!!, 0L,
            struct!!, `layout$OFFSET`, `layout$LAYOUT`.byteSize()
        )
    }
    ...
    private const val `buffer$OFFSET`: Long = 32
