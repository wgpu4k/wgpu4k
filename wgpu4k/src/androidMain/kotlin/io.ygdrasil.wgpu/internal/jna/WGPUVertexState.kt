...
object WGPUVertexState {
    ...
    private val `bufferCount$LAYOUT` = `$LAYOUT`.select(groupElement("bufferCount")) as ValueLayout.OfLong
     ...
     * * }
     */
    fun `bufferCount$layout`(): ValueLayout.OfLong {
        return `bufferCount$LAYOUT`
    }

    private const val `bufferCount$OFFSET`: Long = 40
     ...
     * * }
     */
    fun bufferCount(struct: MemorySegment): Long {
     ...
     * * }
     */
    fun bufferCount(struct: MemorySegment, fieldValue: Long) {
        struct.set(`bufferCount$LAYOUT`, `bufferCount$OFFSET`, fieldValue)
    }

    private val `buffers$LAYOUT`: AddressLayout = `$LAYOUT`.select(groupElement("buffers")) as AddressLayout
     ...
     * * }
     */
    fun `buffers$layout`(): AddressLayout {
        return `buffers$LAYOUT`
    }

    private const val `buffers$OFFSET`: Long = 48
     ...
     * * }
     */
    fun buffers(struct: MemorySegment): MemorySegment {
     ...
     * * }
     */
    fun buffers(struct: MemorySegment, fieldValue: MemorySegment?) {
        struct.set(
            `buffers$LAYOUT`, `buffers$OFFSET`,
            fieldValue!!
        )
    }
