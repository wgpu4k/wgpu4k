...
import io.ygdrasil.wgpu.internal.jna.WGPUBlendComponent
import io.ygdrasil.wgpu.internal.jna.WGPUBlendState
import io.ygdrasil.wgpu.internal.jna.WGPUColorTargetState
import io.ygdrasil.wgpu.internal.jna.WGPUDepthStencilState
import io.ygdrasil.wgpu.internal.jna.WGPUFragmentState
import io.ygdrasil.wgpu.internal.jna.WGPUMultisampleState
import io.ygdrasil.wgpu.internal.jna.WGPUPrimitiveState
import io.ygdrasil.wgpu.internal.jna.WGPURenderPipelineDescriptor
import io.ygdrasil.wgpu.internal.jna.WGPUStencilFaceState
...
internal fun SegmentAllocator.map(input: RenderPipelineDescriptor) = WGPURenderPipelineDescriptor.allocate(this).also { output ->
    ...
    if (input.layout != null) WGPURenderPipelineDescriptor.layout(output, input.layout.mhandler)
...
fun SegmentAllocator.map(input: RenderPipelineDescriptor.FragmentState.ColorTargetState.BlendState): MemorySegment = WGPUBlendState
...
private fun SegmentAllocator.map(input: RenderPipelineDescriptor.FragmentState): MemorySegment =
    WGPUFragmentState.allocate(this)
        .also { fragmentState ->
            println("fragment $fragmentState")
            WGPUFragmentState.module(fragmentState, input.module.mhandler)
...
private fun SegmentAllocator.map(input: RenderPipelineDescriptor.DepthStencilState): MemorySegment =
...
private fun SegmentAllocator.map(input: RenderPipelineDescriptor.VertexState, output: MemorySegment) {
    println("vertex $output")
    WGPUVertexState.module(output, input.module.mhandler)
...
private fun SegmentAllocator.map(input: RenderPipelineDescriptor.VertexState.VertexBufferLayout, output: MemorySegment) {
