        ...
        confined { arena ->
            ...
            WGPUBindGroupLayoutDescriptor.entries(actual).let { entries ->
                WGPUBindGroupLayoutEntry.asSlice(entries, 0).let { entry ->
                    ...
                    WGPUBindGroupLayoutEntry.buffer(entry).let { binding ->
                        WGPUBufferBindingLayout.type(binding) shouldBe BufferBindingType.uniform.value
                        WGPUBufferBindingLayout.hasDynamicOffset(binding) shouldBe 1
                        WGPUBufferBindingLayout.minBindingSize(binding) shouldBe 2
                    }
                    WGPUBindGroupLayoutEntry.sampler(entry) shouldBe MemorySegment.NULL
                    WGPUBindGroupLayoutEntry.texture(entry) shouldBe MemorySegment.NULL
                    WGPUBindGroupLayoutEntry.storageTexture(entry) shouldBe MemorySegment.NULL
                }
                ...
                WGPUBindGroupLayoutEntry.asSlice(entries, 1).let { entry ->
                    ...
                    WGPUBindGroupLayoutEntry.buffer(entry) shouldBe MemorySegment.NULL
                    WGPUBindGroupLayoutEntry.texture(entry) shouldBe MemorySegment.NULL
                    WGPUBindGroupLayoutEntry.storageTexture(entry) shouldBe MemorySegment.NULL
                }
